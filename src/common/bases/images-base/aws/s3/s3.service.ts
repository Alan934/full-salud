...
export abstract class S3Service<T extends ImageBase> {
  ...
  async uploadFile(fileBuffer: Buffer, fileName: string, mimeType: string): Promise<T> {
    try {
      const fileKey = `${this.prefix}/${uuidv4()}`;
      const uploadParams = new PutObjectCommand({
        Bucket: this.bucketName,
  ...
  async uploadFiles(files: { buffer: Buffer; originalname: string; mimetype: string }[]): Promise<T[]> {
    try {
      if (!files || files.length === 0) {
        throw new ErrorManager('No files provided', 400);
      }
