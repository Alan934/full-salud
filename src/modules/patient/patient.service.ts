  ...
  async update(id: string, updatePatientDto: UpdatePatientDto): Promise<Patient> {
    try {
      ...
      if (hasUniqueData) {
        ...
        const orConditions = [];
        if (updatePatientDto.email) orConditions.push({ email: updatePatientDto.email });
        if (updatePatientDto.dni) orConditions.push({ dni: updatePatientDto.dni });
        ...
        if (orConditions.length > 0) {
            ...
            if (conflictingPatient) {
                ...
                if (updatePatientDto.email && conflictingPatient.email === updatePatientDto.email) conflictingField = 'email';
                else if (updatePatientDto.dni && conflictingPatient.dni === updatePatientDto.dni) conflictingField = 'DNI';
        ...
        if (updatePatientDto.email) practitionerOrConditions.push({ email: updatePatientDto.email });
        if (updatePatientDto.dni) practitionerOrConditions.push({ dni: updatePatientDto.dni });

        if (practitionerOrConditions.length > 0) {
          const conflictingPractitioner = await this.practitionerRepository.findOne({
            where: practitionerOrConditions,
          });

          if (conflictingPractitioner) {
            let conflictingField = '';
            if (updatePatientDto.email && conflictingPractitioner.email === updatePatientDto.email) conflictingField = 'email';
            else if (updatePatientDto.dni && conflictingPractitioner.dni === updatePatientDto.dni) conflictingField = 'dni';
      ...
      if (socialWorkEnrollmentId) {
          const socialWorkEnrollment = await this.socialWorkEnrollmentRepository.findOne({ where: { id: socialWorkEnrollmentId } });
          if (!socialWorkEnrollment) {
              throw new ErrorManager(`Social Work Enrollment with ID ${socialWorkEnrollmentId} not found`, 404); // Not Found
          }
          patient.socialWorkEnrollment = socialWorkEnrollment;
      } else if (updatePatientDto.hasOwnProperty('socialWorkEnrollmentId') && socialWorkEnrollmentId === null) {
