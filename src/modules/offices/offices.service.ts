...
> {
  constructor(
    @InjectRepository(Office) protected officeRepository: Repository<Office>
  ) {
  ...
  async getAll(page: number = 1, limit: number = 10): Promise<{ offices: Office[]; total: number; page: number; limit: number; previousPage: number | null; }> {
    try {
  ...
  async getOne(id: string): Promise<Office> {
    try {
